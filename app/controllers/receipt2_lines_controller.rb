 ...
 # def update
  ...
  def update
    ...
    if receipt_line.update(receipt_line_params)
      ...
      render json: { success: false, errors: receipt_line.errors.full_messages }, status: :unprocessable_entity
    end
  end
  ...
  def receipt_line_params
    params.permit(:account_type, :account_no)
  end
  
 def account_options
    options = case params[:account_type]
              when 'bank'
                Bank.pluck(:bank_name)
              when 'customer'
                Customer.pluck(:customer_name)
              when 'g/l_account'
                ChartOfAccount.pluck(:name)
              else
                []
              end
